...
/// Truncate response at sentence boundary (period, question mark, exclamation)
/// without cutting off mid-word
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::thread::spawn(move || {
        if let Err(e) = midi_listener(recording2, pedal_down2, audio2) {
...
fn process(samples: Vec<f32>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    rt.block_on(async {
        ...
        if !text.trim().is_empty() {
            ...
            // System prompt for short audio-only responses
            ...
            if let Ok(parsed) = serde_json::from_str::<OpenClawResponse>(&reply_text) {
                if let Some(choice) = parsed.choices.first() {
                    ...
                    // Truncate to 2 sentences for audio
                    let short_reply = truncate_to_sentences(full_reply, 2);
                    
                    log(&format!("💬 Alan (truncated): {}", short_reply));
                    
                    log_conversation(&transcript, &short_reply);
                    Command::new("/home/jb/bin/speak").arg(&short_reply).spawn().ok();
                }
            }
        } else {
